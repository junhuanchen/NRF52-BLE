...
struct Wearfit
{
    ...
    static void scan_callback(ble_gap_evt_adv_report_t *report)
    {
        ...
        if (true && 0 == memcmp((const char *)report->peer_addr.addr, "\xF8\x2A\xD8\x07\x37\xF6", 6)) // MAC F8-2A-D8-07-37-F6
        {
    ...
    void loop()
    {
        if (Serial.available())
        {
            ...
            if ('S' == tmp.charAt(0))
            {
                // Serial.print((char *)tmp.substring(1, tmp.length()).c_str());
                // set_notice("连接成功");
                set_notice((char *)tmp.substring(1, tmp.length()).c_str());
            }
        }
    }
};